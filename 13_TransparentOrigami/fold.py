...
class Matrix:
    ...
    def count_active_cells(self) -> int:
        return sum(map(lambda r: len(list(filter(lambda c: c, r))), self.cells))
...
# ******************************************
# PART 1 - Fold plastic transparent sheet
# How many dots are visible after completing just the first fold instruction on your transparent paper?
# ******************************************
# Init matrix
matrix = Matrix(fileResult.maxX, fileResult.maxY)
# matrix.print(-1)
print(fileResult.coords)
matrix.fill_coords(fileResult.coords)

# Perform fold
fold = fileResult.folds[0]
print(f"performing fold {fold}")
merged = matrix.fold(fold)

activeCells = merged.count_active_cells()
merged.print(-1, -1)
print(f"total active cells after 1st fold {activeCells}")